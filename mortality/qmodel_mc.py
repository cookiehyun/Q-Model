...
# Paths / config
# ------------------------------------------------------------
BASE_DIR    = "/user/gaad2403/MDS-ED/key/Final/modality"
...
PROB_THRESHOLDS = np.round(np.arange(0.005, 0.036, 0.001), 3)
...
train_prob_icu = train_mean[train_mask]; train_var_icu = train_var[train_mask]; train_ent_icu = train_ent[train_mask]
train_true_icu = train_labels[train_mask].astype(int)

val_prob_icu  = val_mean[val_mask];   val_var_icu  = val_var[val_mask];   val_ent_icu  = val_ent[val_mask]
...
summary_path = os.path.join(CSV_DIR, "prob_thr_sweep_summary_mcdropout_mortality1d_only_mask_trainQ.csv")
