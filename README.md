...
### 5.1 prob_thr ranges where baseline satisfies both sensitivity ≥ 0.80 and specificity ≥ 0.80

| Base model | prob_thr range |
|---|---|
| BasicMLP | 0.08 – 0.10 |
| Deep Ensemble | 0.08 – 0.11 |
| MC Dropout | 0.10 – 0.13 |
| XGBoost | 0.08 – 0.09 |

### 5.2 Best operating point within that range (minimizing absolute FP + FN)

| Base model | Best prob_thr | Q-model strategy | FP | FN | FP+FN |
|---|---|---|---|---|---|
| BasicMLP | 0.09 | Simple XGB | 727 | 143 | 870 |
| Deep Ensemble | 0.10 | Simple XGB | 693 | 139 | **832** |
| MC Dropout | 0.11 | CrossFit XGB | 761 | 142 | 903 |
| XGBoost | 0.09 | Simple XGB | 844 | 146 | 990 |

**Conclusions**
- Across all 4 base models, the **XGB-based Q-model** consistently performs best.
- The **Simple** strategy edges out CrossFit for BasicMLP and Deep Ensemble, while **CrossFit** edges out Simple for MC Dropout.
- Ranked by total error (FP + FN): **Deep Ensemble (832) > BasicMLP (870) > MC Dropout (903) > XGBoost (990)**.

> [!NOTE]
> The table above uses "minimum absolute FP + FN within the sens/spec ≥ 0.80 region" as the final selection criterion. Rankings could differ under an FP-reduction-percentage criterion, so it's worth explicitly stating the chosen criterion in the README.
