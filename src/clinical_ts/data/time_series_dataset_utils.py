...
def load_dataset(target_root,filename_postfix="",df_mapped=True):
    ...
    if(df_mapped):
        df = pd.read_csv(str(target_root)+"/mds_ed.csv", low_memory=False, dtype=object)
        for col in df.columns:
            try:
                df[col] = pd.to_numeric(df[col])
            except: pass
        df = df.apply(lambda x: x.astype(object) if hasattr(x.dtype, "storage") else x)
        df["data"] = df["general_data"]
        df["study_id"] = df["general_study_id"]
    else:
